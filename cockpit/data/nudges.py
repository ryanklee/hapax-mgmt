    ...
    try:
        ...
        for team in health.teams:
            if team.larson_state == "falling-behind":
                evidence = "; ".join(team.larson_evidence) if team.larson_evidence else "multiple signals"
                nudges.append(Nudge(
                    category="people",
                    priority_score=75,
                    priority_label="high",
                ...
                nudges.append(Nudge(
                    ...
                    detail=f"Larson state: treading-water, 0 active coaching experiments",
    ...
    try:
        ...
        for p in snap.people:
            ...
            if p.last_career_convo:
                try:
                    last_dt = datetime.strptime(p.last_career_convo, "%Y-%m-%d").replace(tzinfo=timezone.utc)
