...
class TestDiscoverDueMeetings:
    def test_weekly_stale_returns_due(self):
        snap = _make_snapshot(people=[
            PersonState(name="Alice", cadence="weekly", last_1on1=_days_ago(7),
                        days_since_1on1=7, stale_1on1=True),
        ])
    ...
    def test_weekly_fresh_not_due(self):
        snap = _make_snapshot(people=[
            PersonState(name="Bob", cadence="weekly", last_1on1=_days_ago(3),
                        days_since_1on1=3, stale_1on1=False),
        ])
        with patch("agents.meeting_lifecycle.collect_management_state", return_value=snap):
            from agents.meeting_lifecycle import discover_due_meetings
            due = discover_due_meetings()
    ...
    def test_biweekly_threshold(self):
        snap = _make_snapshot(people=[
            PersonState(name="Dana", cadence="biweekly", last_1on1=_days_ago(14),
                        days_since_1on1=14),
        ])
        with patch("agents.meeting_lifecycle.collect_management_state", return_value=snap):
            from agents.meeting_lifecycle import discover_due_meetings
    ...
    def test_person_filter(self):
        snap = _make_snapshot(people=[
            PersonState(name="Alice", cadence="weekly", last_1on1=_days_ago(7),
                        days_since_1on1=7),
            PersonState(name="Bob", cadence="weekly", last_1on1=_days_ago(7),
                        days_since_1on1=7),
        ])
        with patch("agents.meeting_lifecycle.collect_management_state", return_value=snap):
            from agents.meeting_lifecycle import discover_due_meetings
...
class TestProcessMeeting:
    ...
    async def test_process_meeting_returns_extraction(self, tmp_path: Path):
        ...
        _write_note(note, {"type": "meeting"}, "# Meeting\n- Action: fix bug\n- Decision: use Python")
...
class TestWeeklyReview:
    def test_weekly_review_with_people(self):
        snap = _make_snapshot(people=[
            PersonState(name="Alice", cadence="weekly", last_1on1=_days_ago(15),
                        stale_1on1=True, days_since_1on1=15,
                        cognitive_load=4, coaching_active=True),
            PersonState(name="Bob", cadence="weekly", last_1on1=_days_ago(3),
                        cognitive_load=2),
        ])
        with patch("agents.meeting_lifecycle.collect_management_state", return_value=snap):
...
class TestWriteWeeklyReviewToVault:
    def test_writes_review_file(self):
        ...
        from agents.meeting_lifecycle import (
            WeeklyReviewData, write_weekly_review_to_vault,
        )
        data = WeeklyReviewData(
